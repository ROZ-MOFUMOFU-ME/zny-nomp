...
このポータルは、一体で開発される3リポジトリ構成の最上位です。**3リポジトリとも統合ブランチは `dev` で、開発作業は `dev` 上で行います**（`main` は安定版／リリースブランチ。本リポジトリの `main` は旧 CommonJS の v1.2.x 系のままで、`dev` が ESM の v1.3.0 系です）。兄弟リポジトリは隣のディレクトリにローカルクローンがある前提です:

```
zny-nomp (このリポジトリ — ポータル本体, ESM)
  └─ stratum-pool      = git: ROZ-MOFUMOFU-ME/node-stratum-pool#dev   → ローカルクローン: ../node-stratum-pool (ESM)
...
- 依存関係は**ローカルパスではなく GitHub の `#dev` ブランチに固定**されています。素の `npm install` は GitHub からクローンするため、`../node-stratum-pool` や `../node-multi-hashing` での編集は、`dev` に push して再インストール（`npm update stratum-pool`）するまで反映されません。`package-lock.json` は各 git 依存の特定コミットを固定します。
...
- **プロフィットスイッチャー** (`libs/profitSwitch.js`) — 同一アルゴリズムのコイン間でハッシュパワーを切り替えます（`stratum-pool` のユーティリティを使用。`main` にあった取引所価格 API モジュール群は `dev` では削除済み）。
- **CLI リスナー** (`libs/cliListener.js`) — `scripts/cli.js` からのコマンドを受け付ける TCP ポート（例: デーモンの `blocknotify` フックからのブロック通知）。

Redis が主要データストアです（シェア・ブロック・残高・統計）。プロセス間通信はすべて `init.js` が処理するクラスタ IPC メッセージ経由です。
