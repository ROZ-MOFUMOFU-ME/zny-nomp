...
npm run check:config  # config_example.json / config.json / coins/*・coins/coins-examples{,-testnet}/*・pool_configs/*・pool_configs/examples/* を JSON として parse 検証（実体は test/config.test.ts。npm run test:unit にも含まれる・CI 用）
# 管理コマンド reloadpool / coinswitch は POST /api/admin/<method>（adminCenter.password 認証）で送信。ブロック検知は各プールの p2p。
```
...
コイン定義（`coins/*.json`）はアドレス→スクリプト変換用の `mainnet`／`testnet`（`pubKeyHash`／`scriptHash`／`bech32`／`bip32.public` を **16進文字列**で指定。bech32/P2SH を使うコインは必須。`addressToScript` は network 未指定だと「バージョンバイトを無視した base58 P2PKH」へフォールバックする。koto は `kotoAddressToScript` を使うため network ブロック不要、kumacoin は旧 Peercoin 系で BIP32 が無いため省略）と、デーモン互換フラグ `getInfo`／`noNetworkInfo`／`noGetnetworkhashps` を持ちます。プールアドレスの所有確認（`paymentProcessor` の validateAddress／validateTAddress）が使う RPC は、起動時に daemon を1回プローブして自動判別する — `getaddressinfo`（Bitcoin Core 0.17+。`validateaddress` から `ismine` 等のウォレット項目が外れた）か `validateaddress`（旧 daemon）かを、`-32601`（method not found）の有無という能力ベースで選ぶ（altcoin fork は独自バージョン番号で「0.17」境界にマップしないためバージョン比較は不使用）。設定不要で、旧 `BTCover17` プール設定フラグは廃止した。PoS/PoW ハイブリッドのコイン（VIPSTARCOIN・KumaCoin など）は `getmininginfo.networkhashps` が（はるかに難しい）PoS 難易度を含んで桁違いに大きく出る（VIPSTARCOIN で 568 TH/s）ため、プールの Network H/s と Luck が壊れます。`networkHashFromDiff: true` を立てると、`paymentProcessor` の `cacheNetworkStats` が `networkHash` を生 PoW 難易度由来（`networkDiff × 2^32 ÷ blockTime`、algo multiplier 非依存）で算出して `coin:stats` に書き、stats/metrics はそれをそのまま読みます（純 PoW コインは未設定のまま `networkhashps` を使用）。`getnetworkinfo` 非対応で `subversion`（P2P ユーザーエージェント）を返さない旧ウォレット（kumacoin など。`getinfo.version` は `v0.8.9.9-c60962c-dirty` のようなビルド文字列）向けに、任意の `subVersion` テンプレート（例 `"/Antenna:{version}/"`）を指定すると、`paymentProcessor` がそれを `networkSubVersion` として合成し（`{version}` = version から先頭 `v`・末尾 `-<hash>` を除いた値）、stats の Daemon 表示を本来の `/Antenna:0.8.9.9/` 形式に整えます（実 `subversion` を返すコインではそちらが優先）。任意の `miningTools`（`{name, url}` の配列。URL 文字列だけでも可、`http(s)` のみ表示）を置くと `explorer`（`txURL`／`blockURL`／`address`。`address` は worker stats ページでマイナーアドレスをエクスプローラーへリンク）と同様に `GET /api/config` で公開され、Getting Started ページで該当コインを選択したときに「Mining Software」リンクとして表示されます（マイニングソフトの配布先を複数掲載できる）。各コインの雛形は `coins/coins-examples{,-testnet}/` と `pool_configs/examples/` にあり、CI（check:config）で検証されます。`config.json`／`pool_configs/*.json`／`coins/*.json` は `.gitignore` 済み（実運用設定）で、コミットされるのは例のみです。
...
- **プールワーカー** (`src/poolWorker.ts`) — `clustering.forks` 数だけ fork され、各 fork が有効なプールごとに `stratum-pool` インスタンスを1つ実行します。シェア／ブロックはプールの `share` イベント経由で届き、`src/shareProcessor.ts` が Redis に書き込みます。stratum ポートは `pool_configs/*.json` の `ports`（ポート毎の `diff`／`tls`／`varDiff`）で開き、`tls: true` のポートは stratum-pool が `tls.createServer` で TLS 提供する（共有の `tlsOptions` の key=privkey/cert=fullchain を読む。cert を読めない場合は平文にせず起動拒否＝認証情報の平文漏洩を防ぐ）。qtum 系（VIPSTARCOIN）向けに任意の **getwork ブリッジ**（`getwork.enabled` + stratum と同形の `ports`＝diff/tls/varDiff、`tls: true` で https）があり、stratum でジョブを組めない getwork 専用マイナー（公式 ccminer 等）を同じプールに参加させる（HTTP/ステートレスなので forkId 0 単独起動）。**ただし大半のマイナーは平文 `stratum+tcp://`／`http` getwork しか対応しないため主ポートは平文（`tls: false`）推奨**、TLS は対応マイナー向けの追加ポートで（cert は Let's Encrypt の root-only を pool ユーザーが読めるよう deploy hook で配置。詳細は [docs/stratum-tls.md](docs/stratum-tls.md)）。
- **支払い処理** (`src/paymentProcessor.ts`) — 最大のモジュール。一定間隔で Redis からシェアデータを読み、デーモン RPC でブロックの承認を確認し、支払いを送金します。
- **Website** (`src/website.ts`) — Express 5 サーバー。フロントエンドは `web/` の Vite + React + TypeScript SPA（React Router v7・@tanstack/react-query・recharts・react-i18next による 21 言語 i18n〔旧 translations.json から移植〕・class ベースのライト/ダークテーマ〔`.dark` を `<html>` に付与＋CSS 変数トークン上書き、ヘッダー切替・OS `prefers-color-scheme` 連動・localStorage 永続・index.html の no-flash 初期化〕・モバイル対応〔ハンバーガーナビ・320px まで〕）で、ビルド済み成果物を `web/dist` から配信し、クライアントサイドルーティング用に index.html へフォールバックします（旧 `dot` テンプレート＋jQuery/nvd3 と旧 `website/` フォルダ（index.html・pages・static・key.html）は撤去し、フロントエンドは `web/` に一本化。サーバ側 dot レンダリングも廃止）。JSON API は `src/api.ts`（統計系・`/api/admin`〔`pools`、および `announcement`＝ホームのお知らせ設定。`POST /api/admin/:method` は `adminCenter.password` で認証〕に加え `/api/prices`・公開の `/api/announcement`・`/api/metrics`〔Prometheus, `src/metrics.ts`〕・`/api/health`〔`src/health.ts`〕、さらに SPA が消費する公開ランタイム設定〔stratumHost・有効な切替ポート・プールごとの coin/ports/explorer/miningTools、およびオペレーターのブランディング `website.branding`（サイト名・ヘッダー/ヒーロー別ロゴ・favicon・tagline・`children` でドロップダウン化できる navLinks・Tab Stats ナビ（`/tbs`）の表示可否 `showTabStats`〔未指定＝プールが複数の時だけ表示＝単一コインは自動非表示、true/false で明示上書き〕・home〔ヒーロー見出し〔`title` は coin より優先＝マルチコイン用〕／`No KYC` 等の highlights〔未指定はコード側デフォルト〕／任意 HTML の sections／拠点カード＋ブラウザ側 ping の servers〕・GA4＋任意 script の analytics）〕を返す `GET /api/config`）と `src/workerapi.ts`、Redis からの統計集計は `src/stats.ts`（最新価格を `stats.prices` に付与）。お知らせは Redis キー `announcement` に保存。
...
- **プロフィットスイッチャー** (`src/profitSwitch.ts`) — 同一アルゴリズムのコイン間でハッシュパワーを切り替えます。価格は Redis の `priceFeed:prices` から読み、`reward × price / difficulty` で最良コインを選び、`coinswitch` コマンド（`cliListener` の cliPort へ直接接続）で切り替えます。純粋な選定ロジックは `src/profitSwitchLogic.ts` に分離（テスト可能）。デフォルト無効・価格フィード前提。
- **CLI リスナー** (`src/cliListener.ts`) — profitSwitch の `coinswitch` コマンドを受け付ける内部 TCP ポート（`cliPort`）。手動の reloadpool / coinswitch は website の管理 API（`POST /api/admin`）が master 経由で同じ `dispatchCliCommand`（init.ts）を呼ぶ（旧 `scripts/cli.ts` は廃止）。
